...
## Revisión completa del código (2026-09-18)

- [ ] **Correr `database/031_endurecimiento_seguridad.sql` en el SQL Editor de Supabase** — cierra huecos de seguridad de la base (un cajero podía anular/borrar ventas saltándose el permiso, cualquier usuario podía crearse un negocio con el plan que quisiera, el dueño podía adoptar cuentas ajenas por UUID) y corrige `fn_anular_venta` / `fn_recibir_orden_compra` (doble clic duplicaba el stock). Es re-ejecutable; no se pudo probar contra la base desde acá, así que hay que correrla y revisar que no dé error
- [x] Panel: crédito sin cliente ya no deja la deuda sin anotar, atajo F2 con datos viejos, teléfonos normalizados (0981… = 595981…), zona horaria de turnos manuales, fichaje abierto buscado en la base, fechas de vencimiento que se veían un día antes, arqueo de caja que contaba tarjeta/transferencia/crédito como efectivo, importador que multiplicaba por 100 precios con decimales, exportaciones CSV con celdas escapadas
- [x] Backend: limitador de intentos por usuario (no por IP), contraseñas temporales con generador seguro, un texto escrito ya no se toma como botón, tope de espera en el agrupado de mensajes, lista de espera se restablece si no se pudo enviar la oferta
- [ ] **Pendiente hasta tener crédito de Anthropic y la documentación de Meta:** simulador de conversaciones y tests automáticos del bot, pruebas de uso real, migrar el webhook de Meta a Render
- [ ] Conocidos sin arreglar: Finanzas corta en 300 movimientos y Ventas en 200 (avisa solo en Ventas); "semana"/"mes" son ventanas móviles de 7/30 días, no calendario; la lectura de `sueldo`/`documento` de empleados es visible para todo el equipo (RLS de `usuarios`); un negocio suspendido (`activo=false`) todavía puede entrar al panel; `config` del negocio se actualiza con leer-y-escribir (no atómico)
- [ ] **Pendiente (decidido 2026-09-18): tests y monitoreo.** Orden acordado: (1) UptimeRobot + Sentry (gratis, ~1 hora; las cuentas las crea Arturo), (2) tests automáticos de agenda, permisos, ventas/crédito y arqueo de caja (~6-10 h), (3) pantalla interna de "salud del bot" (mensajes fallidos, recordatorios no enviados, tokens por vencer, costo de API por cliente), (4) simulador de conversaciones del bot — este último recién con crédito de Anthropic y documentación de Meta
- [ ] **Correr `database/032_fotos_lectura_equipo.sql`** (después de la 031): la 031 sacó la política pública de lectura del bucket de fotos y, sin querer, subir la foto de un producto con `upsert` dejó de funcionar. La 032 le da al equipo de cada negocio permiso de leer/actualizar solo las fotos de su carpeta
- [x] Logo del negocio: se sube en Configuración → Apariencia (PNG/JPG/SVG hasta 1 MB) y aparece en la barra lateral del escritorio; se guarda en `config.logo_url`
