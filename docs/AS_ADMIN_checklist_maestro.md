...
- [x] Crear cuentas de Supabase Auth sin tocar Supabase: antes, tanto agregar un empleado (Equipo) como dar de alta un negocio (AdminNegocios) pedían pegar un UUID que había que copiar a mano del panel de Supabase — un paso que un usuario normal no sabe hacer. Ahora los dos piden solo el **email**, y un endpoint nuevo del backend (`POST /api/crear-cuenta`, en `backend/lib/adminUsuarios.js`) crea la cuenta con la service key (nunca expuesta al navegador) y devuelve una **contraseña temporal** que se muestra una sola vez en pantalla para pasarle a la persona. Necesita `VITE_BACKEND_URL` en el `.env` del panel (agregado) y que el backend esté corriendo y sea alcanzable — **sin probar en vivo**, el backend no tiene `.env` en esta máquina y no se pudo levantar. Falta además una pantalla de "cambiar mi contraseña" (no existe en ningún lado todavía) para que la persona pueda cambiar esa contraseña temporal por una propia
